...
def delete_calendar_event(
    ...
    user_access_token: str = Field(None, description="飞书用户访问令牌；配置后会以用户身份删除日程。"),
):
    """删除指定的日程事件，成功返回删除成功信息，失败返回报错信息"""
    if not has_user_access_token(user_access_token):
        return build_user_token_required_error("删除飞书日程")
    ...
    # 发起请求
    response: DeleteCalendarEventResponse = client.calendar.v4.calendar_event.delete(
        request,
