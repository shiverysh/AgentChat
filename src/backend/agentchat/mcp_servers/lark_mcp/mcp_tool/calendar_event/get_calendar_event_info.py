...
def get_calendar_event(
    ...
    user_access_token: str = Field(None, description="飞书用户访问令牌；配置后会以用户身份读取日程。"),
):
    """获取日程事件详细信息，成功返回日程时间详细信息，失败返回报错信息"""
    if not has_user_access_token(user_access_token):
        return build_user_token_required_error("读取飞书日程详情")
    ...
    # 发起请求
    response = client.calendar.v4.calendar_event.get(request, build_request_option(user_access_token))
