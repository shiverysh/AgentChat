...
- user_access_token：飞书用户访问令牌，当前日历/日程能力必填。未配置时系统会直接返回配置错误，不再回退为应用身份。
...
- user_access_token：飞书用户访问令牌，当前日历能力必填。
...
2. 飞书文档能力允许仅配置 APP_ID / APP_SECRET，但飞书日历、日程、参会人等能力必须额外配置 USER_ACCESS_TOKEN。
3. 对于时间相关的参数，统一使用"YYYY-MM-DD HH:MM"格式。
4. 所有ID类型参数默认都使用open_id格式。
5. 删除操作（如删除日历、删除日程事件）都是不可逆的，请谨慎使用。
